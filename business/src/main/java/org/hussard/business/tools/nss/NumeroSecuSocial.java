...
import org.hussard.business.tools.nss.elements.ElementFactory;
import org.hussard.business.tools.nss.elements.EnumElement;
...
public class NumeroSecuSocial {
     ...
     *
     * @param nir le numéro de sécurité social
     * @return true si nb carac. == 15 et la clef valide
     */
    public boolean isValidate(String nir) {
        if (NumeroSecuSocialTools.isSize(nir)) {
            return NumeroSecuSocialTools.isValidateKey(nir);
        }
        return false;
    }
    ...
    public NirInformations parse(String nir) {
        ...
        return NirInformations.aNew()
                .sexe(getValue(EnumElement.SEXE, nir))
                .anneeNaiss(getValue(EnumElement.ANNEE, nir))
                .moisNais(getValue(EnumElement.MOIS, nir))
                .departementNaiss(getValue(EnumElement.CODE, nir))
                .numCommuneNaiss(getValue(EnumElement.NUMERO, nir))
                .numOrdreNaiss(getValue(EnumElement.ORDRE, nir))
                .cleCtrNir(getValue(EnumElement.CLE, nir))
                .create();

    }

    private String getValue(EnumElement enumElement, String nir) {
        return ElementFactory.get(enumElement).value(nir);
    }
}
